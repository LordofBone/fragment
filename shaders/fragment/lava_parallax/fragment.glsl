...
float proceduralDisplacement(vec2 coords) {
    if (useCheckerPattern) {
        // Checker pattern
        float cellCount = 5.0;// adjust as needed
        ...
        h = sign(h)*pow(abs(h), 2.0);// Try adjusting exponent for more contrast
...
void main()
{
    ...
    // Use original TexCoords.x and TexCoords.y for wave patterns
    ...
    // Lava color logic (from original lava shader)
    ...
    vec3 color = mix(lavaColor, envColor, fresnel * 0.2);
